 ...
 * and only the 1xN and 2xN sizes up to eight studs. Names and design IDs are
 * verbatim from `data/rebrickable/parts.csv`; `scripts/rebrickable.ts` fails
 * the palette build if this list and that file ever disagree.
 *
 * There is no 1x5, 1x7, 2x5 or 2x7 — LEGO does not make them, which is why the
 * lengths jump from 4 to 6. Longer bricks (1x10, 1x12, 1x16, 2x10) are real
 * parts deliberately left out: eleven shapes is the whole orderable catalog for
 * a mosaic, and every color in the palette is guaranteed to be checked against
 * all of it.
 */
export const BRICK_SHAPES: readonly BrickShape[] = [
  { designId: '3005', name: 'Brick 1 x 1', w: 1, h: 1 },
  { designId: '3004', name: 'Brick 1 x 2', w: 2, h: 1 },
  { designId: '3622', name: 'Brick 1 x 3', w: 3, h: 1 },
  { designId: '3010', name: 'Brick 1 x 4', w: 4, h: 1 },
  { designId: '3009', name: 'Brick 1 x 6', w: 6, h: 1 },
  { designId: '3008', name: 'Brick 1 x 8', w: 8, h: 1 },
  { designId: '3003', name: 'Brick 2 x 2', w: 2, h: 2 },
  { designId: '3002', name: 'Brick 2 x 3', w: 3, h: 2 },
  { designId: '3001', name: 'Brick 2 x 4', w: 4, h: 2 },
  { designId: '2456', name: 'Brick 2 x 6', w: 6, h: 2 },
  { designId: '3007', name: 'Brick 2 x 8', w: 8, h: 2 },
];
...
/** Flat builds can use the whole catalog. */
export const DEFAULT_FLAT_INVENTORY: readonly string[] = BRICK_SHAPES.map(
  (s) => s.designId
);
 ...
 */
export const DEFAULT_WALL_INVENTORY: readonly string[] = WALL_SHAPES.map(
  (s) => s.designId
);
